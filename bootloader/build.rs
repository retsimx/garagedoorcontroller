...
fn validate(text: &str) {
    ...
    validate_partitions_present(&regions);
    validate_within_flash(&regions);
    validate_page_alignment(&regions);
    validate_dfu_layout(&regions);
    validate_no_overlap(&regions);
    validate_symbols(text);
}

fn validate_partitions_present(regions: &HashMap<String, Region>) {
...
fn validate_within_flash(regions: &HashMap<String, Region>) {
    for name in PARTITIONS {
...
fn validate_page_alignment(regions: &HashMap<String, Region>) {
...
fn validate_dfu_layout(regions: &HashMap<String, Region>) {
...
fn validate_no_overlap(regions: &HashMap<String, Region>) {

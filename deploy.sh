#!/usr/bin/env bash
#
# deploy.sh — build, publish and trigger a garagedoorcontroller OTA release.
#
...
Build the garagedoorcontroller ACTIVE image, publish it for OTA, then trigger
the device to update. The version comes from the repository-root VERSION file.
...
Required environment:
  ...
  DEPLOY_TRIGGER_TOPIC   reset topic (default: garagedoor/reset)

...
/// Upper bound on the whole response-head block, including the status line.
const MAX_HEAD_BYTES: usize = 2048;

/// The validated status and length of a response head.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct ResponseHead {
    /// HTTP status code.
    pub status: u16,
    /// Declared body length; `None` only for a 404 (no body expected).
...
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum HeadError {
    /// The status line does not start with `HTTP/1.0 ` or `HTTP/1.1 `.
    MalformedStatusLine,
    /// The status line has fewer than three space-separated tokens.
    TooFewStatusTokens,
    /// The status code is not numeric.
    NonNumericStatus,
    /// The status code is neither 2xx nor 404.
    UnexpectedStatus,
    /// A `Transfer-Encoding` header was present.
    TransferEncoding,
    /// No `Content-Length` was present on a non-404 response.
    MissingContentLength,
    /// `Content-Length` appeared more than once.
    DuplicateContentLength,
    /// A header line was malformed (e.g. no colon, bad `Content-Length`).
    MalformedHeader,
    /// The header block exceeded [`MAX_HEAD_BYTES`].
    OversizedHeader,
}

/// Incremental result of pushing a byte into a [`HeadParser`].
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum HeadEvent {
    /// The head is incomplete; keep pushing.
    NeedMore,
    /// The head is complete and valid.
    Complete(ResponseHead),
    /// The head is invalid; no further bytes are consumed.
    Reject(HeadError),
}
...
/// Allocation-free HTTP response-head parser fed one byte at a time.
...
impl HeadParser {
    /// Creates a parser awaiting the first status-line byte.
    pub fn new() -> Self {
        Self {
            state: State::Status,
            terminal: None,
    ...
    /// Feeds one byte, returning the parser state after it.

...
/// Why a streamed update did not commit.
#[derive(Debug, PartialEq, Eq)]
pub enum UpdateError<FE, RE> {
    /// The declared body is larger than the active partition.
    Oversize { length: u64, capacity: u32 },
    /// The body ended before `content_length` bytes arrived.
    TooShort,
    /// The reader reported more bytes than were requested.
    TooLong,
    /// The streamed SHA-256 did not match the expected digest.
    HashMismatch,
    /// The flash sink failed.
    Flash(FE),
    /// The body source failed.
    Read(RE),
}

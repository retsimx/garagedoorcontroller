...
//!
//! The RP2040's documented hardware entropy source is the ring-oscillator
//! `RANDOMBIT` register (the same source the Pico SDK's `pico_rand` conditions
//! on). `embassy_rp::clocks::RoscRng` samples that register and implements the
...
//! ## Trust posture
//!
//! The verifier is a no-op: the transport is encrypted and the image is
//! SHA-256-verified, but the server's chain of trust is not validated. This is
//! the user-approved posture from the design (matches the production reference).
...
/// hard-wires `Signature = p256::ecdsa::DerSignature`, which links the ECDSA
/// *signing* types even though they are never executed. Replacing the signature
/// type with a trivial one keeps the same no-verify behaviour without that dead
/// code, which matters for the FLASH budget. This does not remove the whole
/// P-256 stack: TLS 1.3 ECDHE still negotiates the secp256r1 named group, so the
/// P-256 field arithmetic remains linked for the key exchange.
...
pub(super) async fn resolve(stack: NetStack, uri: &Uri<'_>) -> Result<IpEndpoint, OtaError> {
    ...
    match with_timeout(DNS_TIMEOUT, stack.dns_query(uri.host, DnsQueryType::A)).await {
        Ok(Ok(addrs)) => {
            ...
            defmt::warn!("ota_dns_no_address");
            Err(OtaError::Dns)
        }
        Ok(Err(_)) => {
            defmt::warn!("ota_dns_failed");
            Err(OtaError::Dns)
        }
        Err(_) => {
            defmt::warn!("ota_dns_timeout");
...
pub(super) async fn open<'a>(
    ...
    match with_timeout(CONNECT_TIMEOUT, socket.connect(endpoint)).await {
        Ok(Ok(())) => {}
        Ok(Err(_)) => {
            defmt::warn!("ota_connect_failed");
            return Err(OtaError::Connect);
        }
        Err(_) => {
            defmt::warn!("ota_connect_timeout");
    ...
    // Defensive explicit advertisement: `TlsConfig::new` already adds the RSA
    // schemes when the `alloc` feature is compiled in, so this is redundant but
    // harmless (the duplicates sit inside the fixed signature-scheme set). It is
    // kept deliberately because the real endpoint presents an RSA certificate
    // and the handshake cannot be exercised in CI: RSA-PSS must be advertised or
    // the handshake fails before any verification runs.
    ...
    .await
    {
        ...
        Ok(Err(_)) => {
            defmt::warn!("ota_tls_failed");
            Err(OtaError::Tls)
        }
        Err(_) => {
            defmt::warn!("ota_tls_timeout");
            Err(OtaError::Timeout)
        }
    }
}
...
/// Streaming body source for [`apply_update`](garagedoor_core::ota::apply_update):

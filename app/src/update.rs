//! App-side `embassy-boot` firmware-update wrapper (GDC-3, retsimx/garagedoorcontroller#4).
//!
//! Every flash erase, write and read reloads the shared hardware watchdog owned
//! by [`crate::watchdog`], so a long update cannot trip a reset.
//!
...
pub const FLASH_SIZE: usize = garagedoor_core::flash::FLASH_BYTES as usize;
...
/// Longest single blocking flash operation observed, in microseconds.
/// This is the worst-case outage during which both cores are parked.
